...
type Request interface {
	GetKey() int64
	// Check to ensure a request being taken from queue is still valid to process.  Return an error if
	// the request is no longer valid.
	Valid() error
	// Does receiver request represent a newer request than provided request.  Return an error if
	// receiver request is superceded, nil error if it supersedes the provided request.
	Supersedes(Request) error
	// All concluding work/persistence/cleanup once request is processed.
	Finalize() error
}
...
func (m *beginMessage) request() Request {
	return m.req
}
...
func (m *endMessage) request() Request {
	return m.req
}
